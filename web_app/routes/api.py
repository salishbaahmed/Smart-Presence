...
def enroll_student():
    ...
    try:
        ...
        log.info(f"Enrollment started for: {name}")
        if ',' in image_b64:
        ...
        log.info(f"Image decoded: {img.shape}")
        rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        
        log.info("Detecting faces using Haar Cascades...")
        ...
        log.info("Generating face encoding...")
        encodings = face_recognition.face_encodings(rgb, boxes)
        log.info(f"Encoding successful: {len(encodings) > 0}")
        if not encodings:
